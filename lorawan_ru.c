...
static const ChannelParams_t DefaultChannelsRU864[] = {
LC0_RU864,
LC1_RU864,
LC2_RU864,
LC3_RU864,
...
static void InitDefaultRU864Channels (void);
...
void LORAWAN_Reset (IsmBand_t ismBandNew)
{
    ...
    int8_t j;
    set_s("POWER",&p);
    for(j=TXPOWERRU864_MAX;j>=TXPOWERRU864_MIN;j--)
    {
        loRa.txPower = j;
        if(p<=txPowerRU864[j]) break;
    }
//    loRa.txPower = 1; 
...
void LORAWAN_TxDone(uint16_t timeOnAir)
{
    if (loRa.macStatus.macPause == DISABLED)
    {
        ...
        for (i=0; i < MAX_RU_SINGLE_BAND_CHANNELS; i++)
        {
